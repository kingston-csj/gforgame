...
func (ps *HeroService) Init() {
	...
	eventbus.Default().Subscribe(events.PlayerAfterLoad, func(data interface{}) {
...
func (ps *HeroService) DoLevelUp(p *player.Player, heroId int32, toLevel int32) *protos.ResHeroLevelUp {
	...
	eventbus.Default().Publish(events.HeroLevelUp, &events.HeroLevelUpEvent{
		PlayerEvent: events.PlayerEvent{
			Player: p,
		},
		HeroId: heroId,
...
func (ps *HeroService) ReCalculateHeroAttr(p *player.Player, hero *player.Hero, notify bool) {
	...
	eventbus.Default().Publish(events.PlayerAttrChange, p)
}

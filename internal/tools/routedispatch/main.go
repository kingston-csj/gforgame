...
func main() {
	...
	routeDir := filepath.Join(root, "internal", "route")
	methods, err := parseRouteMethods(routeDir, reqCmdMap)
	if err != nil {
		panic(err)
	}

	constructors, err := parseRouteConstructors(routeDir)
	if err != nil {
		panic(err)
	}

	outFile := filepath.Join(root, "cmd", "game", "route_dispatch_gen.go")
	if err := writeGeneratedFile(outFile, methods, constructors); err != nil {
		panic(err)
	}
	fmt.Printf("静态路由表生成完成，输出文件：%s\n", outFile)
}
...
func parseRouteConstructors(routeDir string) ([]routeConstructor, error) {
	files, err := filepath.Glob(filepath.Join(routeDir, "*.go"))
	if err != nil {
		return nil, err
	}

	result := make([]routeConstructor, 0)
	fset := token.NewFileSet()
	for _, filePath := range files {
		fileAst, err := parser.ParseFile(fset, filePath, nil, parser.ParseComments)
		if err != nil {
		...
		for _, decl := range fileAst.Decls {
			...
			if !strings.HasPrefix(fd.Name.Name, "New") || !strings.HasSuffix(fd.Name.Name, "Route") {
				continue
			}
			if fd.Type.Params != nil && len(fd.Type.Params.List) > 0 {
				continue
			}
			result = append(result, routeConstructor{FuncName: fd.Name.Name})
		}
	}
	...
	return deduplicateConstructors(result), nil
}

func deduplicateConstructors(constructors []routeConstructor) []routeConstructor {
	seen := make(map[string]struct{}, len(constructors))
	result := make([]routeConstructor, 0, len(constructors))
	for _, c := range constructors {
		if _, ok := seen[c.FuncName]; ok {
			continue
		}
		seen[c.FuncName] = struct{}{}
...
func parseReqParam(params *ast.FieldList) (reqType string, hasIndex bool, hasSession bool) {
	...
	for _, field := range params.List {
		...
		if sel, ok := star.X.(*ast.SelectorExpr); ok {
			...
			if ok && pkgIdent.Name == "network" && sel.Sel.Name == "Session" {
				hasSession = true
				continue
			}
		}
...
func writeGeneratedFile(filePath string, methods []routeMethod, constructors []routeConstructor) error {
	var b bytes.Buffer
	b.WriteString("// Code generated by internal/tools/routedispatch. DO NOT EDIT.\n\n")
	...
	for _, m := range methods {
		...
		b.WriteString("\t\t}\n")
		if m.HasReturn {
			b.WriteString(fmt.Sprintf("\t\treturn r.%s(%s), nil\n", m.MethodName, buildRouteCallArgs(m)))
		} else {

...
- 基于CSP原语实现的actor并发模型，玩家，排行榜，公会均视为一个actor，每个actor都有自己的goroutine，互不干扰。
- 可选择使用网关模式，游戏客户端通过网关与服务器通信，亦可选择客户端直连方式。
...
提供两种方式，在开发与生产环境下推荐使用不同的连接方式。

- 网关模式下，游戏客户端需要先连接到网关，再通过网关与服务器通信。
  网关相当于一个中间件，负责处理客户端与服务器之间的通信。生产环境下，建议使用网关模式。
  当服务器出现故障时需要热部署进程的时候，网关会自动把所有的消息缓存起来，等服务器恢复后，再把缓存的消息转发送给服务器。
  如此一来，客户端无需重新连接服务器，即可继续与服务器进行通信。
- 直连模式下，游戏客户端直接与服务器通信，开发环境下建议使用直连模式，少起一个进程，便于调试。
